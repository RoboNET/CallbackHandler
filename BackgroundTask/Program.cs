...
builder.Services.AddTransient<AuthResultHandler>();
builder.Services.RegisterCallbackHandler(configurationBuilder =>
{
    configurationBuilder.WithType<AuthResult>(handlerConfigurationBuilder =>
        handlerConfigurationBuilder.WithCacheHandler<AuthResultHandler>());
    configurationBuilder.WithType<ConfirmResult>();
});
...
app.MapGet("/result",
    async () => { return "Processed:" + TestTask.Result; });
